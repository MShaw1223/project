        ...
        {menuOptions.map((opt) => (
          <Link href={`${opt.value}?li=${li}`} key={opt.name}>
            ...
            >
              ...
              <span className="my-1 pl-2 sm:my-2 sm:pl-3 text-slate-100 text-sm sm:text-base md:text-lg lg:text-xl">
                {opt.name}
              </span>
            </div>
          </Link>
...
export const CollapsedOptions = ({ li, isActive }: OptProps) => {
  return (
    <>
      <div className="h-screen flex flex-col items-center justify-center space-y-1">
        {menuOptions.map((opt) => (
          <Link href={`${opt.value}?li=${li}`} key={opt.name}>
            <div
